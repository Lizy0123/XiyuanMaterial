...
@property (nonatomic, copy) LLImageAddItemClickedBlock addItemBlock;
/** 总的媒体数组 */
...
-(void)observeAddItemSelected:(LLImageAddItemClickedBlock)addItemBlock{
    _addItemBlock = addItemBlock;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _mediaArray.count) {
        ...
        cell.LLClickDeleteButton = ^(NSIndexPath *cellIndexPath) {
            ...
            NSLog(@"---%ld",_mediaArray.count);
            
            if (!_allowMultipleSelection) {
                if (_selectedImageAssets.count==0&&_selectedImageModels.count==0) {
                ...
                if (_selectedVideoModels.count ==0) {
            ...
            NSLog(@"-删除一条后-_mediaArray--%@",_mediaArray);
