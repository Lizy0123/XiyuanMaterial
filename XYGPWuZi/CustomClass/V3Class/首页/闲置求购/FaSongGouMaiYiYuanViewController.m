//
//  FaSongGouMaiYiYuanViewController.m
//  XYGPWuZi
//
//  Created by 河北熙元科技有限公司 on 2017/12/26.
//  Copyright © 2017年 河北熙元科技有限公司. All rights reserved.
//

#import "FaSongGouMaiYiYuanViewController.h"

@interface FaSongGouMaiYiYuanViewController ()<UITextViewDelegate>
@property(nonatomic,strong)UITextView *myTextView;

@end

@implementation FaSongGouMaiYiYuanViewController
...
-(void)sendMessage{
    ...
    if (self.myTextView.text.length>0) {
        if (self.myTextView.text.length >512) {
            ...
            [dict setObject:self.riId forKey:@"riId"];
            [dict setObject:kStringToken forKey:@"token"];
            [dict setObject:self.lmTitle forKey:@"lmTitle"];
            [dict setObject:self.myTextView.text forKey:@"lmContent"];
            NSLog(@"--发送参数---%@",dict);
            [[AFHTTPSessionManager manager] POST:[myBaseUrl stringByAppendingString:kPath_LeaveMessage] parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
