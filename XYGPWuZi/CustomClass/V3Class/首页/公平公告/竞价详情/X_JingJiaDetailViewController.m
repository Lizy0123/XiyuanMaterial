//
//  X_JingJiaDetailViewController.m
//  XYGPWuZi
//
...
@interface X_JingJiaDetailViewController ()<UITableViewDelegate, UITableViewDataSource, oneCellBtnClickDelegate, UINavigationControllerDelegate>
//tableView
...
@implementation X_JingJiaDetailViewController
...
-(void)configTableView{
    ...
    UIEdgeInsets insets = UIEdgeInsetsMake(30, 0, kSafeAreaBottomHeight, 0);
...
-(void)configBottomBtn{
    UIEdgeInsets insets = UIEdgeInsetsMake(30, 0, kViewAtBottomHeight, 0);
    ...
    if (!self.bottomBtn) {
        self.bottomBtn = [UIButton buttonWithStyle:StrapDefaultStyle andTitle:titleStr andFrame:CGRectMake(kMyPadding, 0, kScreen_Width - kMyPadding *2, 44) target:self action:@selector(actionBottomBtn:)];
...
-(void)configHeaderView{
    //创建tabble的头部
    self.headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 30)];
    self.headerView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [self.view addSubview:_headerView];
    
    _leftImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 1, S_W/3, 29)];
    [self.headerView addSubview:_leftImage];
    
    _leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 1, (S_W/3)-30, 29)];
    _leftLabel.textColor = [UIColor whiteColor];
    _leftLabel.font = [UIFont systemFontOfSize:15];
    _leftLabel.textAlignment = NSTextAlignmentCenter;
    [self.headerView addSubview:_leftLabel];
    
    self.endTimeLabel = [[UILabel alloc]initWithFrame:CGRectMake(S_W/3, 0, 2*S_W/3, 30)];
    [self.headerView addSubview:self.endTimeLabel];
    self.endTimeLabel.textAlignment = NSTextAlignmentCenter;
    X_JJDetailFirstModel *model = [self.firstModelArray lastObject];
    
    NSLog(@"tobegin-%ld---ongoing-%ld----isend-%@",model.tooBegin,model.onnGoing,model.isEnd);
    if ([model.isEnd isEqualToString:@"1"]) {
        ...
        if (model.tooBegin >0) {
            _secondsCountDown = model.tooBegin/1000;  //倒计时秒数
            _leftImage.image = [UIImage imageNamed:@"kaiShiAndEndBackground"];
            ...
            if (model.onnGoing>0) {
                _twoSecondsCountDown = model.onnGoing/1000 ;  //倒计时秒数
...
-(void)requestData{
    ...
    [[AFHTTPSessionManager manager] POST:[myBaseUrl stringByAppendingString:kPath_TradeNoticeDetail] parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        X_JJDetailFirstModel *amodel = [[X_JJDetailFirstModel alloc]init];
        //isEnd：是否结束（0、否 1、是）
        amodel.isEnd = responseObject[@"object"][@"isEnd"];
        amodel.tooBegin = [responseObject[@"object"][@"toBegin"] integerValue];
        amodel.onnGoing = [responseObject[@"object"][@"onGoing"] integerValue];
        ...
        if (weakSelf.isShowBottomView) {
            if (amodel.tooBegin >0) {
                ...
                if ([weakSelf.isEntry integerValue] == 0) {
                    ...
                    self.bottomBtn.enabled = YES;
                    [weakSelf.bottomBtn setTitle:@"报名" forState:UIControlStateNormal];
                    //右上角按钮
//                    UIBarButtonItem *nextStepBtn = [[UIBarButtonItem alloc]initWithTitle:@"出价记录" style:UIBarButtonItemStyleDone target:self action:@selector(goToBiddingRecord)];
...
//    if ([self.isEntry integerValue] == 1) {
//        [self goToDetailVcWithTsId:self.tsId];
//    }else{
        [self actionBaoming];
//    }
}
...
//    NSLog(@"定时器在运行");
    ...
    if(_secondsCountDown<=0){
        ...
        X_JJDetailFirstModel *model = self.firstModelArray[0];
        _threeSecondsCountDown = (model.onnGoing - model.tooBegin)/1000;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        if (indexPath.row == 0) {
            ...
            oneCell.cellType = detailOneCellTypeDefault;
