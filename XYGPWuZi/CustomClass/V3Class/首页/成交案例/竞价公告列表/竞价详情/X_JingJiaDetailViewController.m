...
-(void)setupTableView{
    ...
    UIButton *button = [UIButton buttonWithStyle:StrapDefaultStyle andTitle:@"报  名" andFrame:CGRectMake(16, 0, S_W-32, 44) target:self action:@selector(baoMing)];
...
-(void)requestData{
    ...
    NSString *url = [requestUrlHeader stringByAppendingString:JJDetailUrl];
    NSLog(@"---url===%@",url);
    ...
    [manager POST:url parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        for (NSDictionary *dict in (NSArray *)responseObject[@"object"][@"xpiList"]) {
...
-(void)baoMing{
    ...
    if ([kStringToken length]) {
        ...
        if ([memberEntity.facIsTrade isEqualToString:@"0"]) {
            ...
            NSString *url = [requestUrlHeader stringByAppendingString:baoMingJoinJingJiaUrl];
            ...
            [manager POST:url parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
                ...
                if (codeStr == 200) {
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.7 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        XRJingJialistViewController *vc = [[XRJingJialistViewController alloc]init];
                        vc.defaultPage = 0;
