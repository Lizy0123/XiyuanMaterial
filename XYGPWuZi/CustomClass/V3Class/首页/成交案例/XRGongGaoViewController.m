...
#import "XRGongGaoViewController.h"
...
@interface XRGongGaoViewController ()<UINavigationControllerDelegate>
@property (nonatomic, strong) NSMutableArray *allVC;
@property (nonatomic, strong) SMPagerTabView *segmentView;
@end

@implementation XRGongGaoViewController
...
- (void)viewDidLoad {
    ...
    JingJiaListViewController *one = [[JingJiaListViewController alloc]initWithNibName:nil bundle:nil];
    one.nav = self.navigationController;
    one.title = @"竞价公告";
    ...
    two.nav = self.navigationController;
    two.title = @"交易预告";
    
    [_allVC addObject:two];
    
    XRChengJiaoAnLiViewController *three = [[XRChengJiaoAnLiViewController alloc]initWithNibName:nil bundle:nil];
    three.title = @"成交案例";
    three.nav = self.navigationController;
