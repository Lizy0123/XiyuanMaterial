...
#define kTextSelectColor kColorNav
            ...
            if (haveItems) {
                ...
                if (_dataSourceFlags.titleForRowAtIndexPath) {
                    if (_dataSourceFlags.imageNameForRowAtIndexPath) {
                        NSString *imageName = [_dataSource menu:self imageNameForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:0]];
                        if (imageName && imageName.length > 0) {
                            ...
                            _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, _dropDownViewWidth, self.customView.frame.size.height + kMyPadding *2);
    ...
    if (_dataSourceFlags.titleForRowAtIndexPath) {
        if (_dataSourceFlags.imageNameForRowAtIndexPath) {
            ...
            if (imageName && imageName.length > 0) {
                ...
                return self.customView.frame.size.height + kMyPadding *2;
            }
        }else {
            return kTableViewCellHeight;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _leftTableView) {
        ...
        //Lzy_定制CellView
        if (self.hasCustomView&&cell.imageView.image == nil) {
